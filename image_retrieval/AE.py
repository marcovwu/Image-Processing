...
parser.add_argument('--protopath',   type=str,   default='./collection_for_1052/collection_for_1052',   help='Prototype path')
parser.add_argument('--headpath',   type=str,   default='./collection_for_1052/collection_for_1052',     help='Head image path')
parser.add_argument('--resume_ename',   type=str,   default='./models/e_AE_F1.pkl',             help='Head image path')
parser.add_argument('--resume_dname',   type=str,   default='./models/e_AE_F1.pkl',             help='Head image path')
    ...
    if int(train)==0:
        ...
        for i in range(args.epoch):
            ...
            for j,(proto,image) in enumerate(train_loader):
                ...
                print('epoch:%d aug_data--batch:%d/%d Loss:%.4f' %(i+1,j+1,int(365/args.batch_size)+1,float(loss/res.numel())))
            ...
            for j,(proto,image) in enumerate(train_loader):
                ...
                optimizer.step()
                sum_loss+=loss/res.numel()
                print('epoch:%d smaller stn_data--batch:%d/%d Loss:%.4f' %(i+1,j+1,int(365/args.batch_size)+1,float(loss/res.numel())))
            if loss/res.numel()<best:
                print('Saving modle...')
                best=loss/res.numel()
                index=i                
            ...
            average_loss.append(sum_loss/(j+1)/2)  
            print(' -Average_distance: ',round(float(sum_loss/(j+1)/2),4),"",'in epoch :',i+1)
            if sum_loss/(j+1)/2<best_average:
                print('Saving best_average modle...')
                best_average=sum_loss/(j+1)/2
                avg_index=i                
                torch.save(encoder.state_dict(), args.encode_name)  
                torch.save(decoder.state_dict(), args.decode_name)
                excute=0
